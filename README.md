...
Additionally, the hardware requires commands to be sent individually (chunk size of 1) with a 0.1s delay between payloads. The integration handles this throttling and chunking automatically (configured via `THROTTLE_DELAY` and `CMD_CHUNK_SIZE` in `const.py`), utilizing an `asyncio.Lock()` to prevent overlapping socket connections when multiple commands are triggered simultaneously.
...
*Note: All features and hardware limits have been successfully tested, verified, and documented up to version 0.4.29.*
